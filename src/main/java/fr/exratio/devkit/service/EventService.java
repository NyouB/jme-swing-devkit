...
import fr.exratio.devkit.event.EventHandler;
import fr.exratio.devkit.event.EventListener;
import fr.exratio.devkit.event.EventManager;
import fr.exratio.devkit.event.EventPriority;
import fr.exratio.devkit.event.EventThreadingException;
import fr.exratio.devkit.event.MethodContainer;
...
public class EventService implements EventManager, Service {
  ...
  private final EnumMap<EventPriority, Map<Class<? extends Event>, List<MethodContainer>>> eventListenerMap = new EnumMap<>(
      EventPriority.class);
   ...
   *
   * @param eventListener the EventListener to register.
   * @throws EventThreadingException if the event was registered on a thread other than the main
   * thread.
   */
  @Override
  public void tryRegisterEventListener(EventListener eventListener) throws EventThreadingException {
    ...
    for (final Method method : methods) {
      ...
      List<MethodContainer> eventMethods = stuff
   ...
   *
   * @param eventListener the EventListener to unregister.
   * @throws EventThreadingException if the event was unregistered on a thread other than the main
   * thread.
   */
  @Override
  public void tryUnregisterEventListener(EventListener eventListener)
      throws EventThreadingException {
   ...
   *
   * @param event the event to send to all listeners.
   * @throws EventThreadingException if the event was not fired in accordance with its threading
   * status. - A synchronous event must be fired from the main thread. - An asynchronous event must
   * not be fired from synchronous code. - An asynchronous event must not be fired from the main
   * thread.
   */
  @Override
  public void tryFireEvent(Event event) throws EventThreadingException {
    ...
    if (event.isAsync()) {
      ...
      if (Thread.holdsLock(event)) {
        throw new EventThreadingException(event.getClass().getSimpleName()
            + " IGNORED. Event is marked as asynchronous but has been fired from synchronous code.");
      }

      if (isPrimaryThread()) {
        throw new EventThreadingException(event.getClass().getSimpleName()
            + " IGNORED. Event is marked as asynchronous but has been fired from the main thread.");
      }
      ...
      if (!isPrimaryThread()) {
        throw new EventThreadingException(event.getClass().getSimpleName()
            + " IGNORED. Event is marked as synchronous but has been fired from a different thread.");
      }

    }

    // the iteration will be in the same order as the enum
    for (Map.Entry<EventPriority, Map<Class<? extends Event>, List<MethodContainer>>> entry : eventListenerMap
        .entrySet()) {
